...
function buildDriverScript() {
    ...
    L.push('    if (!modal.querySelector(".gpp-palette-grid") || modal.querySelector(".gpp-palette-grid").classList.contains("gpp-palette-list-mode")) throw new Error("compact mode did not render its independent Grid layout");');
    ...
    L.push('    if (!compactHandle.classList.contains("gpp-resize-active")) throw new Error("REGRESSION: compact resize corner did not show active feedback while resizing");');
    L.push('    await new Promise(function(resolve) { setTimeout(resolve, 220); });');
    L.push('    if (!/rgba?\\(\\s*(34,\\s*197,\\s*94|166,\\s*227,\\s*161)/.test(getComputedStyle(compactHandle).backgroundColor)) throw new Error("REGRESSION: compact resize corner active feedback is not green after its fade-in: " + getComputedStyle(compactHandle).backgroundColor);');
    L.push('    if (!getComputedStyle(compactHandle).transitionProperty.includes("background-color")) throw new Error("REGRESSION: compact resize corner feedback does not fade via background-color transition");');
    ...
    L.push('    if (compactHandle.classList.contains("gpp-resize-active")) throw new Error("REGRESSION: compact resize corner kept active feedback after resizing ended");');
    ...
    L.push('    function assertCompactVisible(selector, label) {');
    L.push('      var element = modal.querySelector(selector);');
    L.push('      if (!element) throw new Error("REGRESSION: compact view is missing " + label + " (" + selector + ")");');
    L.push('      var style = getComputedStyle(element);');
    L.push('      var rect = element.getBoundingClientRect();');
    L.push('      if (style.display === "none" || style.visibility === "hidden" || Number(style.opacity) === 0 || rect.width <= 0 || rect.height <= 0) {');
    L.push('        var chain = [];');
    L.push('        for (var cursor = element; cursor && cursor !== modal.parentElement; cursor = cursor.parentElement) {');
    L.push('          var cursorStyle = getComputedStyle(cursor), cursorRect = cursor.getBoundingClientRect();');
    L.push('          chain.push({ tag: cursor.tagName, id: cursor.id, className: cursor.className, display: cursorStyle.display, flex: cursorStyle.flex, width: cursorRect.width, height: cursorRect.height, offsetHeight: cursor.offsetHeight });');
    L.push('        }');
    L.push('        throw new Error("REGRESSION: compact " + label + " exists but is not visibly laid out: " + JSON.stringify({ display: style.display, visibility: style.visibility, opacity: style.opacity, width: rect.width, height: rect.height, flex: style.flex, chain: chain }));');
    L.push('      }');
    L.push('    }');
    L.push('    assertCompactVisible("#gpp-modal-left", "left panel");');
    L.push('    assertCompactVisible("#gpp-left-body", "left body");');
    L.push('    assertCompactVisible("#gpp-palette-section", "palette section");');
    L.push('    if (!compactPaletteDetails.open) throw new Error("REGRESSION: compact Template Colors details is closed");');
    L.push('    assertCompactVisible("#gpp-palette-section > details > .gpp-body", "palette body");');
    L.push('    assertCompactVisible(".gpp-palette-panel", "palette panel");');
    L.push('    assertCompactVisible("#gpp-palette-bulk-top", "Enable all/Disable all row");');
    L.push('    assertCompactVisible(".gpp-palette-view-row", "Palette view row");');
    L.push('    assertCompactVisible(".gpp-palette-grid", "palette grid");');
    L.push('    minifyBtn.click();');
    ...
    L.push('      if (!resizeHandle.classList.contains("gpp-resize-active")) throw new Error("REGRESSION: full-view resize corner did not show active feedback while resizing");');
    ...
    L.push('      if (resizeHandle.classList.contains("gpp-resize-active")) throw new Error("REGRESSION: full-view resize corner kept active feedback after resizing ended");');
    ...
    L.push('      if (!compactResizeHandle.classList.contains("gpp-resize-active")) throw new Error("REGRESSION: compact-view resize corner did not show active feedback while resizing");');
    ...
    L.push('      if (compactResizeHandle.classList.contains("gpp-resize-active")) throw new Error("REGRESSION: compact-view resize corner kept active feedback after resizing ended");');
    ...
    L.push('      return "full and compact Ghost++ views accept touch-style button=-1 pointer gestures, use touch-action:none on their handles, show transient corner feedback while resizing, move by touch, resize by touch, persist compact dimensions, and remain inside a narrow viewport";');
    L.push('    });');
    L.push('  }');
    L.push('');
    ...
    L.push('    var resizedHostWidth = Math.max(240, bottomWidthBeforeScale - 180);');
    L.push('    bottom.style.width = resizedHostWidth + "px"; window.dispatchEvent(new Event("resize"));');
    L.push('    await new Promise(function(resolve) { requestAnimationFrame(function() { requestAnimationFrame(function() { requestAnimationFrame(resolve); }); }); });');
    L.push('    var surfaceWidthAfterResize = scaleRoot.getBoundingClientRect().width;');
    L.push('    var visualWidthAfterResize = scaleContent.getBoundingClientRect().width;');
    L.push('    var resizedRootStyle = getComputedStyle(scaleRoot);');
    L.push('    var resizedSurfaceContentWidth = surfaceWidthAfterResize - parseFloat(resizedRootStyle.paddingLeft || "0") - parseFloat(resizedRootStyle.paddingRight || "0") - parseFloat(resizedRootStyle.borderLeftWidth || "0") - parseFloat(resizedRootStyle.borderRightWidth || "0");');
    L.push('    if (Math.abs(bottom.getBoundingClientRect().width - resizedHostWidth) > 1 || Math.abs(surfaceWidthAfterResize - resizedHostWidth) > 1 || Math.abs(visualWidthAfterResize - resizedSurfaceContentWidth) > 1) throw new Error("REGRESSION: a scaled Paint Menu Controls surface kept its old pixel width after the host viewport changed: " + JSON.stringify({ expected: resizedHostWidth, expectedContent: resizedSurfaceContentWidth, bottom: bottom.getBoundingClientRect().width, root: surfaceWidthAfterResize, visual: visualWidthAfterResize }));');
    L.push('    var bottomStyleWidthAfterResize = bottom.style.width;');
    ...
    L.push('    if (Math.abs(bottomWidthAfterLargeScale - resizedHostWidth) > 1) throw new Error("REGRESSION: scaling changed #bottomControls outer width at 120% after the host resized (expected=" + resizedHostWidth + ", after=" + bottomWidthAfterLargeScale + ")");');
    L.push('    if (Math.abs(surfaceWidthAt120 - surfaceWidthAfterResize) > 1 || Math.abs(scaleContent.getBoundingClientRect().width - resizedSurfaceContentWidth) > 1) throw new Error("REGRESSION: Paint Menu Controls scale changed the resized surface width at 120% instead of only its height");');
    ...
    L.push('    if (bottom.style.width !== bottomStyleWidthAfterResize) throw new Error("REGRESSION: large-scale commit rewrote the host\'s resized #bottomControls width");');
